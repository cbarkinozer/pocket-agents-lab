...
40. [ ] Re-run Ling at the corrected 2048-token context, then run the 50-case development pilot and compare it with a re-run Qwen reference under exactly the same runtime protocol.
41. [ ] Instrument page faults, file-backed residency, bytes read, and per-case latency before implementing expert-aware caching. `RuntimeTelemetry.kt` now reads `/proc/self/stat`, `/proc/self/io`, and `/proc/self/smaps_rollup` per case and is unit-tested; on-device verification of these specific counters against the real Ling run is still pending (needs the A32 attached).
