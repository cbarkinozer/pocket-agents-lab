...
22. [x] Add a foreground "Hey Agent" voice-control MVP: an opt-in segmented recognition session waits for a sufficiently confident "Hey Agent", plays one acknowledgement tone, allows a longer following command utterance, and runs it through the local agent. Repeating "Hey Agent" re-arms command capture instead of becoming a request. Low-risk media/search voice actions execute directly; messages and state-changing actions still require confirmation. Stop or leaving/locking the app cancels recognition and visibly disables the toggle. Screen-off/background hotword detection remains future work because it requires a foreground microphone service and explicit battery/privacy design.
