...
19. [x] Add a confirmed calendar-event MVP: the SLM selects `create_calendar_event`, Kotlin requires a title plus a today/tomorrow time, and confirmation opens Android's calendar editor with a one-hour draft rather than silently saving it.
...
39. [x] Prove Ling 3.0 Tiny Q3_K_M feasibility on the Galaxy A32: update llama.cpp for `bailingmoe3`, verify the 3.79 GB GGUF hash, load it through file-backed mmap, and complete one local answer. See `docs/results/2026-09-10-ling-q3-android-feasibility.md`.
40. [x] Re-run Ling at the corrected 2048-token context, then run the 50-case development pilot and compare it with a Qwen reference under exactly the same runtime protocol. Ling scored 44/50 and Qwen Q4 scored 41/50 on the completed A32 runs; Qwen was approximately 2.0x faster and used approximately 4.3x less post-case PSS. See `docs/results/2026-09-11-ling-qwen-a32-comparison.md`. Separately, a same-protocol (non-device) replay on Windows via `llama-server` got Ling to 44/50 (88%); an earlier Qwen3.5-0.8B replay swung between 15/50 and 31/50 depending on grammar/prompt variant and remains a harness caveat, not the authoritative device result. See `docs/results/2026-09-11-ling-qwen08-windows-pilot-replay.md`.
41. [x] Instrument page faults, file-backed residency, bytes read, and per-case latency before implementing expert-aware caching. `RuntimeTelemetry.kt` reads `/proc/self/stat`, `/proc/self/io`, and `/proc/self/smaps_rollup` per case and is unit-tested; the completed A32 Ling run populated RSS, swap PSS, page-fault, read-byte, latency, and thermal fields. `Pss_File` is null on this Samsung kernel and is preserved as null. See `docs/results/2026-09-11-ling-a32-routing-pilot.md`.
42. [x] Prototype the first Android expert-streaming foundation: persistent GGUF expert offset parsing, bounded native LRU cache, `pread` range reads, JNI controls, async prefetch, and cache telemetry. See `docs/results/2026-09-11-android-expert-cache-mvp.md`.
    ...
    A real (non-synthetic) expert-selection trace was captured on a Windows host from the actual Android system prompt and analyzed with a new `tools/expert_cache_simulator.py`: a bounded per-layer LRU cache hit rate of 26/51/76% at 8/16/32 cached experts (of 128), all well above the random baseline, confirming genuine reusable locality in Ling's routing and justifying the bounded-cache stage. See `docs/results/2026-09-11-ling-q3-windows-feasibility.md`. The native cache, route callback, predictor, and async prefetch are now integrated and device-tested; actual ggml tensor-buffer replacement remains open.
43. [x] Add the opt-in BailingMoe3 scheduler hook and native expert telemetry toggle. The callback ran during real Ling inference on the A32 with unchanged output and reported 230 route events / 81,328 selected expert IDs; automatic route-to-cache prefetch produced 5,657 queued prefetch requests and read real Ling ranges from storage.
44. [x] Add the first simple previous-token expert-set predictor. On the real Ling A32 run it produced 5,729 predictor prefetch requests on top of 230 route events; Top-8 recall and wasted-byte accounting remain to be added.
45. [x] Run the first same-prompt vanilla-vs-route/cache/predictor A/B on the A32: Ling output stayed identical; the pilot measured 4,432 ms vanilla versus 11,749 ms with the current observational prefetch path. Repeat across cache budgets and cold/warm order before claiming a performance benefit.
46. [x] Add opt-in `madvise(MADV_WILLNEED)` page warming against the actual loaded ggml tensor mapping, plus experimental `MADV_DONTNEED` release of experts absent from the previous route. Device inference passes with zero duplicate resident cache bytes; this remains observational because ggml dispatch still owns the tensor pointers.
47. [ ] Optional full-fork experiment: replace normal `MUL_MAT_ID` CPU dispatch with bounded cache-backed tensor buffers. This is intentionally not part of the safe Android path. The supported non-fork approximation is exposed as `ExpertCacheRuntime.enableStorageBacked(...)`: mmap-backed tensors, selected-page warming, previous-route eviction, predictor, cold reset, and `mincore()` residency telemetry. It does not claim bounded-buffer kernel semantics.
48. [x] Compare the safe storage-backed Ling mode with the frozen Qwen Q4 reference on the A32. On the same short prompt, Ling completed in 6,407 ms and Qwen in 2,233 ms; Ling emitted 7,360 direct page-warm and 3,504 page-drop requests with zero duplicate cache bytes. See `docs/results/2026-09-11-ling-storage-backed-qwen-a32-comparison.md`.
49. [x] Run the paired 50-case vanilla-vs-storage-backed Ling experiment on the A32. Vanilla scored 44/50 with 3,393,888 KiB average post-case PSS; storage-backed scored 43/50 with 1,085,531 KiB average PSS, 50/50 strict JSON in both, and no crashes. Storage-backed mode showed a real residency/memory reduction but no accuracy improvement. See `docs/results/2026-09-11-ling-vanilla-vs-storage-backed-50case.md`.
