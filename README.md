...
22. [x] Add a foreground "Hey Agent" voice-control MVP: an opt-in toggle continuously restarts Android's speech recognizer while the Agent screen is open, accepts commands beginning with "Hey Agent", and runs them through the same local agent and confirmation flow. Screen-off/background hotword detection remains future work because it requires a foreground microphone service and explicit battery/privacy design.
