...
40. [ ] Re-run Ling at the corrected 2048-token context, then run the 50-case development pilot and compare it with a re-run Qwen reference under exactly the same runtime protocol. Ling's corrected A32 run is complete (`docs/results/2026-09-11-ling-a32-routing-pilot.md`). A Qwen Q4 rerun was started but remained incomplete at the thermal gate while USB-powered (`docs/results/2026-09-11-qwen-a32-thermal-block.md`); do not score that artifact as a model result. Separately, a same-protocol (non-device) replay on Windows via `llama-server` got Ling to 44/50 (88%); a Qwen3.5-0.8B replay swung between 15/50 and 31/50 depending on which grammar/prompt variant was used, a gap large enough that the Qwen number should be treated as an unreliable harness artifact, not a capability finding, until diagnosed. See `docs/results/2026-09-11-ling-qwen08-windows-pilot-replay.md`. This still does not substitute for the on-device figure.
...
42. [ ] Prototype Android expert-aware prefetch in stages: expert offset index, bounded cache, simple history predictor, async prefetch, and only later a trained prerouter. Keep each stage paired against vanilla mmap.
