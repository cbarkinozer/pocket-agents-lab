...
46. [x] Add an opt-in `madvise(MADV_WILLNEED)` page-warm mode that maps selected expert ranges, touches their pages, and releases the mapping without retaining duplicate buffers. During routing it also advises the selected slices of the actual loaded ggml tensor mapping. This is device-safe and measurable, but remains observational because ggml dispatch still owns the tensor pointers.
47. [ ] Replace observational duplicate reads with actual ggml tensor-buffer/page-cache integration, then repeat cache-budget and cold/warm vanilla A/B measurements. Do not describe page-warm mode as Edge0-equivalent until tensor dispatch consumes the warmed/cache-backed ranges.
