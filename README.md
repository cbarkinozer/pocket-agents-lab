...
47. [ ] Replace observational duplicate reads with actual ggml tensor-buffer/page-cache integration, then repeat cache-budget and cold/warm vanilla A/B measurements. A controlled `releaseAllExpertPages()` reset and `expertResidencyJson()`/`mincore()` signal now report total versus last-route selected resident bytes. Do not describe page-warm/eviction mode as Edge0-equivalent until tensor dispatch consumes bounded cache-backed ranges.
