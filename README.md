...
40. [ ] Re-run Ling at the corrected 2048-token context, then run the 50-case development pilot and compare it with a re-run Qwen reference under exactly the same runtime protocol. As of 2026-09-11 a live on-device run was reportedly in progress (checkpoint `files/agent-routing-1789070616481-Ling-3.0-tiny-Q3_K_M-result.json`); check that checkpoint for `complete: true` before starting another on-device run. Separately, a same-protocol (non-device) replay on Windows via `llama-server` got Ling to 44/50 (88%); a Qwen3.5-0.8B replay swung between 15/50 and 31/50 depending on which grammar/prompt variant was used, a gap large enough that the Qwen number should be treated as an unreliable harness artifact, not a capability finding, until diagnosed. See `docs/results/2026-09-11-ling-qwen08-windows-pilot-replay.md`. This still does not substitute for the on-device figure.
