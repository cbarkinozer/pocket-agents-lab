...
42. [x] Prototype the first Android expert-streaming foundation: GGUF expert offset tooling, bounded native LRU cache, `pread` range reads, JNI controls, async prefetch primitive, and cache telemetry. The loaded-model JNI index currently reports safe architecture metadata only; persistent native GGUF offset parsing remains open. See `docs/results/2026-09-11-android-expert-cache-mvp.md`.
...
43. [x] Add the opt-in BailingMoe3 scheduler hook and native expert telemetry toggle. The callback is compiled and lifecycle-tested; run it during real Ling inference on the A32, compare output parity, then route expert weight reads through the cache without changing model semantics.
