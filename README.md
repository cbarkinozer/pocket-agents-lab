...
46. [x] Add an opt-in `madvise(MADV_WILLNEED)` page-warm mode that maps selected expert ranges, touches their pages, and releases the mapping without retaining duplicate buffers. This is device-safe and measurable, but remains observational because ggml still owns the original mmap tensor pointers.
