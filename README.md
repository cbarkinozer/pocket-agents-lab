...
22. [x] Add a foreground "Hey Agent" voice-control MVP: an opt-in segmented recognition session waits for "Hey Agent", plays one acknowledgement tone, captures the following utterance as the command, and runs it through the same local agent and confirmation flow. Stop cancels recognition and disables the toggle. Screen-off/background hotword detection remains future work because it requires a foreground microphone service and explicit battery/privacy design.
